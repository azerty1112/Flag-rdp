...
class FolderSync {
    ...
    public function uploadFolder($userId, $zipFile, $folderName) {
        if ($zipFile['error'] !== UPLOAD_ERR_OK) {
            ...
            return ['error' => 'فشل رفع الملف', 'details' => 'يرجى التحقق من حجم الملف ورفع نسخة جديدة.'];
        }
        ...
        if ($ext !== 'zip') {
            ...
            return ['error' => 'يُسمح فقط بملفات ZIP', 'details' => 'يرجى رفع ملف ZIP صالح.'];
        }
        if ($zipFile['size'] > MAX_FILE_SIZE) {
            app_log_error('UPLOAD', 'حجم الملف يتجاوز الحد المسموح', __FILE__, __LINE__, null, ['size' => $zipFile['size']]);
            return ['error' => 'حجم الملف كبير جدًا', 'details' => 'الحد الأقصى هو ' . number_format(MAX_FILE_SIZE / 1024 / 1024, 1) . ' ميغابايت.'];
        }
        $uploadDir = __DIR__ . "/../assets/user_folders/{$userId}/";
        if (!is_dir($uploadDir) && !mkdir($uploadDir, 0755, true) && !is_dir($uploadDir)) {
            app_log_error('UPLOAD', 'تعذر إنشاء مجلد المستخدم', __FILE__, __LINE__);
            return ['error' => 'تعذر إنشاء مجلد المستخدم', 'details' => 'تحقق من صلاحيات المجلدات على الخادم.'];
        }
        ...
        if (!move_uploaded_file($zipFile['tmp_name'], $targetPath)) {
            ...
            return ['error' => 'تعذر حفظ الملف', 'details' => 'تحقق من صلاحيات الكتابة على مجلد المستخدم.'];
        }
